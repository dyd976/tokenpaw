    ...
    def test_read_endpoints_return_phase1_data(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            with TestClient(app) as client:
                ...
                self.assertEqual(client.get(f"/api/sessions/{session_id}/insights").status_code, 200)
                ...
                self.assertEqual(detail.json()["likely_cause"]["tool_name"], "shell_command")
                self.assertEqual(detail.json()["likely_cause"]["confidence"], "High")
