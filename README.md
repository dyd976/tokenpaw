...
TokenPaw (AI Agent Token Observatory)는 Claude Code, OpenAI Codex 등 코딩 에이전트의 로컬 로그를 분석해 **모델 → 프로젝트 → 세션 → 프롬프트 → 원인**의 흐름으로 토큰 사용량을 추적합니다.

원문과 소스 코드는 기본적으로 외부로 전송하지 않으며, provider가 기록한 실제 사용량과 로컬 메타데이터 기반 추정값을 구분해 표시합니다.
...
- 실제 provider usage가 없는 이벤트의 token 값은 추정하지 않고 null로 둡니다. tool byte size에서 파생하는 `estimated_tokens`만 `is_estimated=true`로 표시합니다.
...
`GET /api/overview`, `/api/agents`, `/api/projects`, `/api/sessions`, `/api/sessions/{id}`, `/api/sessions/{id}/turns`, `/api/sessions/{id}/timeline`, `/api/sessions/{id}/insights`, `/api/turns/{id}`, `/api/turns/{id}/impact`, `/api/alerts`, `/api/search?q=...`, `/api/usage/timeline`, `/api/usage/models`, `/api/usage/projects`. Overview, agent, project, session, timeline, model, and usage-project endpoints accept `start`/`end` filters; overview/timeline use `agent`, while agent/project/model/usage-project endpoints also accept provider/agent filters. Alerts accept `resolved`, `alert_type`, `severity`, `agent`, `start`, and `end`. Period aggregation uses turn timestamps and normalizes timezone representations before comparing them. The full dashboard's `Today / Last 7 days` and agent controls now reuse these filters for KPI, timeline, sessions, and alerts. Session timeline rows combine provider turns with normalized ContextItem/ToolCall metadata; context/tool token amounts remain explicitly estimated when the provider does not record them separately.

Cost estimation is opt-in through a local YAML/JSON pricing file (`--pricing` or `TOKEN_MONITOR_PRICING_PATH`). Prices are USD per million tokens. If a model or one of its token classes has no configured price, the cost remains `Unknown` instead of being guessed.
...
The desktop popover starts as a small pet. Click to open the diagnostics panel; long-press and drag to move it. The panel is positioned above and to the left of the pet (clamped to the monitor work area) and drills down through `Agent → Project → Session → Prompt / Turn`. The first screen separately shows today’s per-agent usage, then the hierarchy shows input, cached, fresh, output, reasoning, cache-hit, and cost values at each level for the selected `Today / 7 days / All time` period, with an `Actual / Mixed / Estimated` usage-quality label derived from the underlying turns. It also includes an agent-stacked token-burn timeline with `Today / 7 days / All time` and `5m / 15m / 1h / 1d` controls; the hierarchy refreshes every 10 seconds while open. Session view includes a compact turn-input chart, context attribution by type, and diagnostic cards. Turn detail shows `Input Before → Input After`, delta, fresh-context added, reasoning, cost, and Tools/Files/MCP/Subagents counts. Effective input is `cached input + fresh input` when those provider fields exist; provider raw input remains visible as a separate value. Prompt usage is attached to the first measured provider call for that prompt. The desktop bundle stores masked user prompt text locally so prompt-level search and burn analysis work; tool output and file contents remain disabled. CLI/library users can keep prompt storage disabled unless `store_prompt_content` is enabled explicitly.
...
Turn persistent impact is deliberately shown as `Estimated` with low confidence: the adapters use exact content-hash recurrence to find later turns containing the same context, deduplicate each hash once per later turn, and multiply its representative token count by the affected later-turn count. Provider logs do not expose a complete context-to-token attribution graph, so this is a diagnostic signal rather than an actual provider usage value.
...
alert는 `alert type + session + turn` hash를 id로 사용하므로 같은 scan을 반복해도 중복 생성되지 않습니다. 수치는 실제 provider usage와 tool output 크기 기반 추정값을 구분하며, 원인 설명에는 baseline/current metric을 함께 저장합니다.
...
- provider 로그가 전체 context를 원인별 token으로 attribution하지 않으므로 파일·shell·MCP·sub-agent별 수치는 context item/tool output 크기 기반의 `Estimated`입니다. provider usage 자체는 `Actual`로 표시합니다.
- Claude의 tool result와 Codex function output은 크기와 hash를 보호 설정에 따라 저장할 수 있지만, provider가 해당 결과의 실제 token 수를 기록하지 않은 경우 `estimated_tokens`입니다.
